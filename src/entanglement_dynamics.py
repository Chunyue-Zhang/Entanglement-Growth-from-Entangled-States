...
t_ff = np.concatenate(
    (np.linspace(0.0, T1_ff, n1_ff + 1), np.linspace(T1_ff, T1_ff + T2_ff, n2_ff + 1))
)
...
h_hamiltonian = hamiltonian(
    [["xx", Jxff_list], ["yy", Jyff_list]], [], basis=basis, dtype=np.float64
)
...
h_hamiltonian = hamiltonian(
    [["xx", JxF_list], ["yy", JyF_list]], [], basis=basis, dtype=np.float64
)
...
for i in range(4):
    U = np.array(
        [
            [np.exp(-1j * gamma_degree[i] * np.pi / 180 / 4), 0, 0, 0],
            [
                0,
                np.exp(1j * gamma_degree[i] * np.pi / 180 / 4)
                * np.cos(alpha_beta_degree[i] * np.pi / 180 / 2),
                -1j
                * np.exp(1j * gamma_degree[i] * np.pi / 180 / 4)
                * np.sin(alpha_beta_degree[i] * np.pi / 180 / 2),
                0,
            ],
            [
                0,
...
def apply_gate_fast(initial_vector, gate, bond_index):
    ...
    temp_tensor = np.tensordot(
        gate_tensor, initial_tensor, axes=([2, 3], [bond_index, bond_index + 1])
    )
...
for i in range(tsn):
    ...
    for ii in range(n0 + 1):
        ...
        for iii in range(n1_ff + n2_ff + 2):
            ...
            EE_freefermion[ii, iii] = basis.ent_entropy(psi, sssi, density=False)[
                "Sent_A"
            ]
    print("Anderson")
    h_list = [[hs_A[ii], ii] for ii in range(L)]
    h_hamiltonian = hamiltonian(
        [["xx", JxA_list], ["yy", JyA_list], ["zz", JzA_list], ["z", h_list]],
        [],
        basis=basis,
        dtype=np.float64,
    )
    ...
    h_hamiltonian = hamiltonian(
        [["zz", JzF_list], ["z", h_list]], [], basis=basis, dtype=np.float64
    )
    fulle, fullv = h_hamiltonian.eigh()
    ...
    h_hamiltonian = hamiltonian(
        [["xx", JxM_list], ["yy", JyM_list], ["zz", JzM_list], ["z", h_list]],
        [],
        basis=basis,
        dtype=np.float64,
    )
        ...
        for iii in range(rscsn):
            ...
            for iiii in range(n0 + 1):
                ...
                for iiiii in range(D):
                    ...
                    psi = psi_whole[sector_index_in_whole] / np.linalg.norm(
                        psi_whole[sector_index_in_whole]
                    )
                    EE_circuit[ii, iii, iiii, iiiii + 1] = basis.ent_entropy(
                        psi, sssi, density=False
                    )["Sent_A"]
